...
**Hard Mode** is enforced by a separate `HardModeValidator`, checked before a guess is scored - a rejected guess isn't recorded and doesn't cost an attempt. It's baked into a session at creation (like `maxGuesses`) rather than being a live setting: for Daily/Endless it's a `localStorage` preference applied to the *next* fresh game you start, and for Custom it's the creator's choice, stored on the puzzle itself. This means an already-in-progress game always keeps whatever Hard Mode setting it started with, even if you flip the titlebar toggle mid-game.
